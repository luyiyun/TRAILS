...
class FPCAFeaturePipeline:
    ...
    def transform(self, data: ClinicalTimeSeriesDataset) -> NDArray[np.float64]:
        ...
        if self.score_scaler is None:
            raise RuntimeError("FPCAFeaturePipeline必须先拟合")
        return np.asarray(
...
class FPCAKMeansBaseline(FPCAFeaturePipeline):
    ...
    def predict(
        ...
        *,
        ...
        if self.model is None:
            raise RuntimeError("FPCAKMeansBaseline必须先拟合")
